...
globals [
  ...
  ;riot?               ; segnala se è in corso una rivolta
  cumsum              ; cumulativa degli attivi durante stessa rivolta
  riottime            ; tempo di durata della rivolta
  waittime            ; tempo di attesa tra una rivolta e l'altra

  ;;;;; BINS for waiting time distribution ;;;;;
  zerotenbin
  tentwentybin
  twentythirtybin
  thirtyfourtybin
...
to go
  ...
  if legitimacy-slow-drop = True
  [if government-legitimacy > 0 [set government-legitimacy (government-legitimacy - 0.001)]]

  if legitimacy-sudden-drop = True
  [if ticks > 300 [set government-legitimacy 0.7]]
...
to determine-behavior
  ...
  if RNDM [
    ...
    if (rand < random-choice * 100)
    [set active? (rand < random-choice * 100)]
...
to enforce
  if any? (agents-on neighborhood) with [ active? ] [
    ...
    ask suspect [
      ...
      ;;;MYCODE;;;
      if infinite-jail = True [set jail-term 1000000]
      ;;;;;;;;;;;;
    ]
  ]
end
...
to plotting
  let riot? False
  let riottosi (count turtles with [active? = true])
  ifelse (riottosi > 50)
  [set riot? True
    if (waittime < 10) and (waittime > 0) [set zerotenbin (zerotenbin + 1)]
    if (waittime < 20) and (waittime > 9) [set tentwentybin (tentwentybin + 1)]
    if (waittime < 30) and (waittime > 19) [set twentythirtybin (twentythirtybin + 1)]
    if (waittime < 40) and (waittime > 29) [set thirtyfourtybin (thirtyfourtybin + 1)]
    if (waittime < 50) and (waittime > 39) [set fourtyfiftybin (fourtyfiftybin + 1)]
    if (waittime < 60) and (waittime > 49) [set fiftysixtybin (fiftysixtybin + 1)]
    if (waittime < 70) and (waittime > 59) [set sixtyseventybin (sixtyseventybin + 1)]
    if (waittime < 80) and (waittime > 69) [set seventyeightybin (seventyeightybin + 1)]
    if (waittime < 90) and (waittime > 79) [set eightyninetybin (eightyninetybin + 1)]
    if (waittime < 100) and (waittime > 89) [set ninetyhundredbin (ninetyhundredbin + 1)]
    if waittime > 100 [set morethanhundredbin (morethanhundredbin + 1)]

    if (waittime < 5) and (waittime > 0) [set zerofivebin (zerofivebin + 1)]
    if (waittime < 10) and (waittime > 4) [set fivetenbin (fivetenbin + 1)]
    if (waittime < 15) and (waittime > 9) [set tenfifteenbin (tenfifteenbin + 1)]
    if (waittime < 20) and (waittime > 14) [set fifteentwentybin (fifteentwentybin + 1)]
    if (waittime < 25) and (waittime > 19) [set twentytwentyfivebin (twentytwentyfivebin + 1)]
    if (waittime < 30) and (waittime > 24) [set twentyfivethirtybin (twentyfivethirtybin + 1)]
    if (waittime < 35) and (waittime > 29) [set thirtythirtyfivebin (thirtythirtyfivebin + 1)]
    if (waittime < 40) and (waittime > 34) [set thirtyfivefourtybin (thirtyfivefourtybin + 1)]
    if (waittime < 45) and (waittime > 39) [set fourtyfourtyfivebin (fourtyfourtyfivebin + 1)]
    if (waittime < 50) and (waittime > 44) [set fourtyfivefiftybin (fourtyfivefiftybin + 1)]

   set waittime 0]
  [set cumsum 0

   set riottime 0 set waittime (waittime + 1)]

  if riot?
  [
    set cumsum (cumsum + riottosi)
    set riottime (riottime + 1)
  ]
...
"" ""
PENS
"default" 1.0 0 -13345367 true "" "clear-plot\n\nplotxy 10 zerotenbin\nplotxy 20 tentwentybin\nplotxy 30 twentythirtybin\nplotxy 40 thirtyfourtybin\nplotxy 50 fourtyfiftybin\nplotxy 60 fiftysixtybin\nplotxy 70 sixtyseventybin\nplotxy 80 seventyeightybin\nplotxy 90 eightyninetybin\nplotxy 100 ninetyhundredbin\nplotxy 110 morethanhundredbin"
"pen-1" 1.0 0 -1184463 true "" "clear-plot\n\nplotxy 5 zerofivebin\nplotxy 10 fivetenbin\nplotxy 15 tenfifteenbin\nplotxy 20 fifteentwentybin\nplotxy 25 twentytwentyfivebin\nplotxy 30 twentyfivethirtybin\nplotxy 35 thirtythirtyfivebin\nplotxy 40 thirtyfivefourtybin\nplotxy 45 fourtyfourtyfivebin\nplotxy 50 fourtyfivefiftybin\n"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "clear-plot\n\nlet dieci 0\nifelse zerotenbin = 0\n[set dieci 0]\n[set dieci ln(zerotenbin)]\n\nlet venti 0\nifelse tentwentybin = 0\n[set venti 0]\n[set venti ln(tentwentybin)]\n\nlet trenta 0\nifelse twentythirtybin = 0\n[set trenta 0]\n[set trenta ln(twentythirtybin)]\n\n\nlet quaranta 0\nifelse thirtyfourtybin = 0\n[set quaranta 0]\n[set quaranta ln(thirtyfourtybin)]\n\nlet cinquanta 0\nifelse fourtyfiftybin = 0\n[set cinquanta 0]\n[set cinquanta ln(fourtyfiftybin)]\n\nlet sessanta 0\nifelse fiftysixtybin = 0\n[set sessanta 0]\n[set sessanta ln(fiftysixtybin)]\n\nlet settanta 0\nifelse sixtyseventybin = 0\n[set settanta 0]\n[set settanta ln(sixtyseventybin)]\n\nlet ottanta 0\nifelse seventyeightybin = 0\n[set ottanta 0]\n[set ottanta ln(seventyeightybin)]\n\nlet novanta 0\nifelse eightyninetybin = 0\n[set novanta 0]\n[set novanta ln(eightyninetybin)]\n\nlet cento 0\nifelse ninetyhundredbin = 0\n[set cento 0]\n[set cento ln(ninetyhundredbin)]\n\n\nlet centodieci 0\nifelse morethanhundredbin = 0\n[set centodieci 0]\n[set centodieci ln(morethanhundredbin)]\n\nplotxy 10 dieci\nplotxy 20 venti\nplotxy 30 trenta\nplotxy 40 quaranta\nplotxy 50 cinquanta\nplotxy 60 sessanta\nplotxy 70 settanta\nplotxy 80 ottanta\nplotxy 90 novanta\nplotxy 100 cento\nplotxy 110 centodieci"
"pen-1" 1.0 0 -2674135 true "" "clear-plot\n\nplotxy 5 ln(zerofivebin + 1)\nplotxy 10 ln(fivetenbin + 1)\nplotxy 15 ln(tenfifteenbin + 1)\nplotxy 20 ln(fifteentwentybin + 1)\nplotxy 25 ln(twentytwentyfivebin + 1)\nplotxy 30 ln(twentyfivethirtybin + 1)\nplotxy 35 ln(thirtythirtyfivebin + 1)\nplotxy 40 ln(thirtyfivefourtybin + 1)\nplotxy 45 ln(fourtyfourtyfivebin + 1)\nplotxy 50 ln(fourtyfivefiftybin + 1)"
...
368
TOT
zerotenbin + tentwentybin + twentythirtybin + thirtyfourtybin + fourtyfiftybin + fiftysixtybin + sixtyseventybin + seventyeightybin + eightyninetybin + ninetyhundredbin + morethanhundredbin
17
1
11
...
"" ""
PENS
"default" 1.0 0 -10899396 true "" "plot government-legitimacy * 1000"
"pen-1" 1.0 0 -16777216 true "" "plot count agents with [jail-term > 0]"
"pen-2" 1.0 0 -2674135 true "" "plot count turtles with [active? = True]"
